...
def conv2d(name, tensor,ksize, out_dim, stddev=0.01, stride=2, padding='SAME'):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [ksize, ksize, tensor.get_shape()[-1],out_dim], dtype=tf.float32,initializer=tf.random_normal_initializer(stddev=stddev))
...
def train():
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        ...
        path = os.getcwd()
        data_dir = "/tmp/tensorflow/mnist/input_data"#准备使用自己的数据集
        # print data_dir
        '''获得数据'''
