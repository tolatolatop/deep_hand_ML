...
def feed_dict(train,input_port, labels_port, drop_port):
	if train:
		...
		k = 1.0
	return {input_port:xs, labels_port:ys, drop_port:k}
...
with tf.name_scope('input_reshape'):
	iamge_shaped_input = tf.reshape(x, [-1, 28, 28, 1])
	tf.summary.image('input', iamge_shaped_input, 10)

def weight_variable(shape):
	initial = tf.truncated_normal(shape, stddev=0.1)
	tvars = tf.Variable(initial)
	return tvars
...
y = nn_layer(dropped, 500, 10, 'layers', act = tf.identity)
